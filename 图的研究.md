...
+ [ ] **forming online support groups for internet and behavior related addictions**：在社交网络上寻找互助小组，通过在加权图的补图上寻找团的方式寻找满足规模下限的权重最大的群组

  > 后面再详写

+ [ ] **On finding socially tenuous groups for online social networks**：构造k三角度量，贪心寻找k三角最少的群组，并证明了找到的是最优群组之一
...
+ [x] **graph adversarial training_dynamically regularizing based on graph structure**：通过给网络中节点特征加入一个扰动破坏和周围节点的流畅性，生成器用于生成尽可能和原网络相似的扰动项，判别器用于分辨加了和没加扰动项的节点
