...
class SDNE:
    ...
    def __make_loss(self,config):
        ...
        def get_2nd_loss(X, newX, beta):
            B = X * (beta-1) + 1
            return tr.reduce_sum(tf.pow((newX -X)*B, 2))

        def get_reg_loss(weight, biases):
            ret = tf.add_n([tf.nn.l2_loss(w) for w in weight.itervalues()])
            ret = ret + tf.add([tf.nn.l2_loss(b) for b in biases.itervalues()])
            return ret
    ...
    def do_variable_init(self, data):
        def assign(a, b):
            op = a.assign(b)
            self.sess.run(op)
    ...
    def fit(self,data):
        ret,_ = self.sess.run((self.loss, self.optimizer), feed_dict=self.__get_feed_dict(data))
        return ret

    def get_loss(self,data):
        return self.sess.run(self.loss,feed_dict=self.__get_feed_dict(data))
